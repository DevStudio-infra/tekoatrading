...
export function TradingPairDialog({
  ...
  // TRPC queries
  const { data: allPairs, isLoading: isLoadingAll } = trpc.tradingPair.getAll.useQuery(
    {
  ...
  const { data: popularPairs, isLoading: isLoadingPopular } = trpc.tradingPair.getPopular.useQuery(
    {
      brokerName: selectedBroker,
      limit: 50,
    },
  ...
  const { data: searchResults, isLoading: isSearching } = trpc.tradingPair.search.useQuery(
    {
      query: searchQuery,
      limit: 100,
    },
  ...
  const { data: availableCategories } = trpc.tradingPair.getCategories.useQuery(undefined, {
    enabled: open,
  });
  ...
  useEffect(() => {
    if (availableCategories) {
      ...
      const mappedCategories = Array.from(
        new Set(availableCategories.map((cat: string) => categoryMap[cat] || cat)),
      );
        ...
        <ScrollArea className="flex-1">
          {isLoading && results.length === 0 ? (
            ...
            >
              {results
                .filter((pair) => {
                  // Filter out pairs with numeric-only symbols like "0001", "0002", etc.
                  if (/^\d+$/.test(pair.symbol)) return false;
